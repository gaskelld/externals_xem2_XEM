...
c!---------------------------------------------------------------------
c      subroutine FNP_NMC(X,QSQ,rat)
c!---------------------------------------------------------
c! NMC FIT TO NMC,SLAC,NMC data in CERN-PPE/91-167
c! No Fermi Motion Corrections
c!  Steve Rock 1 Feb 1996
c!-------------------------------------------------------------
c      IMPLICIT NONE
c      REAL*8 X,QSQ,A,B,X2,X3,rat
c    
c      X2 = X*X
c      X3 = X2*X
c      A = 0.979 -1.692*X +2.797*X2 -4.313*X3 +3.075*X3*X
cC$$      B = -.171*X2 + .277*X3
c      B = -.171*X2 + .244*X3  ! replaced 10/22/97 by correct value on x3
c      rat = A *(1+X2/QSQ)*(QSQ/20.)**B
c      RETURN
c      END
