...
!--------------------------------------------------------------------
      ...
      avgN = A-z
      if(IA.LT.2) then
c        call sf(WSQ,QSQ,F1p,FLp,F2p,F1n,FLn,F2n)
        call sf(WSQ,QSQ,F1,F2,IZ)
c        if(IZ.LT.1) then        !!! Neutron
c          F1 = F1n
c          F2 = F2n
c        else                    !!! Proton
c          F1 = F1p
...
c      write(6,*) 'cheesy poofs',IZ,F1,F2
...
!!!   Note:  SQESUB MUST be called prior to RESCSD to read in smearing function with first=.true.
...
c      firsty = .true.
      ...
      if(firsty) then
        if(wfn.EQ.1) then
          open(unit=34,file='f1f2tables/f11.WBARELav18',status='old')
          open(unit=35,file='f1f2tables/f2.WBARELav18',status='old')
          open(unit=36,file='f1f2tables/f12.WBARELav18',status='old')
        elseif(wfn.EQ.2) then
c          open(unit=34,file='f1f2tables/f11-onshell-cdbonn.dat',status='old')        
c          open(unit=35,file='f1f2tables/f22-onshell-cdbonn.dat',status='old')
c          open(unit=36,file='f1f2tables/f12-onshell-cdbonn.dat',status='old')
          open(unit=34,file='f1f2tables/f11.WBARELcdbonn',status='old')        
          open(unit=35,file='f1f2tables/f2.WBARELcdbonn',status='old')
          open(unit=36,file='f1f2tables/f12.WBARELcdbonn',status='old')
        elseif(wfn.EQ.3) then
...
CCCC   Converts reduced cross sections to structure functions for protons and neutrons  CCCCC 
      ...
      if(IZ.lt.1) then
         call rescsn(w2,q2,sigT,sigL)      
      else
         call rescsp(w2,q2,sigT,sigL)
      endif

      F1 = sigT/0.3894e3/pi2/alpha/8.0*abs(w2-mp2)
      FL = sigL*2.0*x/0.3894e3/pi2/alpha/8.0*abs(w2-mp2)
      F2 = (2.*x*F1+FL)/(1.+4.*mp2*x*x/q2)

c      write(6,*) 'cheesy poofs', IZ,sigT,sigL,F1,F2

c      f1p = sigTp/0.3894e3/pi2/alpha/8.0*abs(w2-mp2)
c      f1n = sigTn/0.3894e3/pi2/alpha/8.0*abs(w2-mp2)
c      fLp = sigLp*2.0*x/0.3894e3/pi2/alpha/8.0*abs(w2-mp2)
c      fLn = sigLn*2.0*x/0.3894e3/pi2/alpha/8.0*abs(w2-mp2)
c      f2p = (2.*x*f1p+fLp)/(1.+4.*mp2*x*x/q2)
c      f2n = (2.*x*f1n+fLn)/(1.+4.*mp2*x*x/q2)
