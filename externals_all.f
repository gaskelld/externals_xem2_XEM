...
C=======================================================================
      ...
      COMMON /OMG/   DELTA,R
      REAL   DELTA,R                                        
      COMMON /RAD/   ETA,B,AX0,BA,AX0A,ETAC,BC,AX0C,                                 
     >               ATBX0,ATAX0,TB,TA,BTBI,BTAI,BTB,BTA,
     >               TBEFOR,TBAL,TAFTER,TAAL,TBC,TAC   
      REAL   ETA,B,AX0,BA,AX0A, ETAC, BC, AX0C,                                 
     >       ATBX0,ATAX0,TB,TA,BTBI,BTAI,BTB,BTA,
     >               TBEFOR,TBAL,TAFTER,TAAL,TBC,TAC                   
...
C aluminum:                                                             
      ...
      BA   = 1.3667511                                                  
      AX0A =  .0017535 
C add these for cell walls - cryo material assumed to be H/D here
      ETAC  = LOG(1194./1.0**0.6667)/LOG(184.15/1.0**0.3333)               
c      BC    = 4./3.*(1.+1./9.*(1.0+1.)/(iZ+ETA)/LOG(184.15/1.0**0.3333))   
c      AX0C  = 0.000154*1.0/0.93828*0.001575 ! use proton mass here    
      BC    = 4./3.*(1.+1./9.*(1.0+1.)/(1+ETAC)/LOG(184.15/1.0**0.3333))   
      AX0C  = 0.000154*1.0/0.93828*61.28 ! use proton mass/parameters here    
C     TASUM=0.                                                          
C     TBSUM=0.                                                          
...
c         write(*,'(1x,''e,ep,th,sigb,sigqe='',3f8.2,2e12.4)')
c     >        E0SET,EPSET,TH,SIGMA_BORN,sigma_corr
...
C=======================================================================
     ...
     >               TBEFOR,TBAL,TAFTER,TAAL,TBC,TAC
      ...
      INTEGER IZ,IA                                                     
      REAL AVGN,AVGA,AVGM,AMUM
      REAL DELP1,DEL01,DEL02,DELP2,DELP,
...
c XEM 6 GeV
c         TBAL = 0.0125/8.9 + 0.00495 +TBEAM !upstream wall and cryotarget 
         TBAL = 0.001575 ! upstream wall, avg. of Loop2 and 3
     >          + TBEAM
         TBC = 0.01203 ! avg. of LH2 and LD2 

         TAAL = TSPEC
         
         TBEFOR=t
         TAFTER = (ttarg-t)/cos(thr) ! approximate as flat piece of Al
      ENDIF
...
c        cell_diam = 3.96
c        cell_wall = 0.0125
c! z in cm relative to center 
c        ztarg = -0.5*cell_diam 
c        call tunatarg(cell_diam, cell_wall, 
c     >    ztarg, thr, cryo_cm, wall_cm)
c        TAAL = 0.00495 * cryo_cm / cell_diam +wall_cm/8.9 + TSPEC
         TWALL=0.00220
         ECIR=1.3123*2.54        ! endcap inner radius (cm
         ECOR=1.320*2.54        ! endcap outer radius (cm
         TARGLEN=3.942*2.54     ! target length (cm
         ENTEC=TARGLEN-ECIR     ! entrance to end cap (cm
                                               ! vertex
        if(ECIR/tan(thr).lt.ENTEC) then  ! e goes through sidewall
          TAAL=TSPEC+TWALL/sin(thr)    ! side wall
          TAC=(0.01203/targlen)*ECIR/sin(thr) ! cryotarget material
        else
          TAC=                              ! e goes throught end cap
     >     (sqrt(ECIR**2-((targlen-ECIR)*sin(thr))**2)
     >    +(targlen-ECIR)*cos(thr))*0.01203/targlen ! liquid target
          TAAL=TSPEC                           ! material after the target
     >    +(sqrt(ECOR**2-((targlen-ECIR)*sin(thr))**2)
     >    -sqrt(ECIR**2-((targlen-ECIR)*sin(thr))**2))
     >    *TWALL/(ECOR-ECIR)                   ! & end cap
        endif
...
c         write(6,*) 'cheesy poofs ',TARGET,TBAL,TAAL,TBEFOR,TAFTER
      ...
      ATBX0 = AX0*TBEFOR+AX0A*TBAL+AX0C*TBC                                      
      ATAX0 = AX0*TAFTER+AX0A*TAAL+AX0C*TAC                                      
      BTBI  = B*TBEFOR+BA*TBAL+BC*TBC                                         
      BTAI  = B*TAFTER+BA*TAAL+BC*TAC                                          
...
C Equivalent radiator for internal bremstrahlung correction             
      ...
      TB  = TBEFOR+TBAL+TBC+TEQUI                                           
      TA  = TAFTER+TAAL+TAC+TEQUI                                           
      BTB = B*(TBEFOR+TEQUI)+BA*TBAL+BC*TBC                                    
      BTA = B*(TAFTER+TEQUI)+BA*TAAL+BC*TAC                                    
...
cdg      write(17,*) '***Ebeam    Eprime  Theta     x        Q2'//
cdg     > '      Sig_Born     Sig_Born_In  Sig_Born_QE  Sig_Rad'//
cdg     > '      Sig_Rad_EL   Sig_Rad_QE   Sig_Rad_DIS  C_cor'
...
!      COMMON       /TARGT/ iZ, iA, avgN, avgA, avgM, amuM               
      ...
      REAL*8 VW2,X,EMCFAC
      REAL*8    C(24),CF(11),CN(11),CD(24),CFD(11)                          
      REAL*8    EF(7) 
      REAL FITEMC
...
C OVERCOME RISK OF UNDERFLOW IN THE EXPONENTIATION                      
      ...
      IF (amuM.LE.1.5 .and. amuM.GE.0.5) THEN !hydrogen
C          UNIVERSAL AND RESONANCE FIT FOR HYDROGEN                     
           UNIV = SLACF2(W,QQ,CF)                                       
           BRES = B(W,QQ,C)
      ELSEIF (amuM.LT.0.5) then
           UNIV = SLACF2(W,QQ,CN)                                       
           BRES = B(W,QQ,C)
      ELSE                                                              
...
!------------------------------------------------------------------------
C removed nform to get rid of conflicts with other version
